...
### Un utilitaire UPLOAD de chargement local de fichiers
...
### Un utilitaire d'administration d'EXPORT d'un espace
...
- à l'ouverture d'une connexion, seuls les documents ayant évolué sur le serveur par rapport à l'état connu dans la b&ase IDB (ou ajoutés depuis la dernière connexion) sont téléchargés: cette phase peut s'en trouver grandement accélérée;
...
La base IDB ne stocke pas par défaut les fichiers attachés aux notes en raison du volume possiblement important correspondant. Toutefois, fichier par fichier, l'utilisateur peut les faire stocker localement. 
...
- des actions de _saisie_ des paramètres des opérations, puis une fois satisfait de sa saisie il déclenche une _validation_ qui soumet l'opération correspondante au service OP ...  
- ce qui en général provoque, après vérifications et calculs, des mises à jour de la base ...
- ce qui provoque en retour des avis de modification de documents du périmètre et déclenche une opération Sync pour obtenir les documents modifiés ...
...
> Remarque: l'export de base `firestore` vers `firestore` étant techniquement limité par les contraintes d'environnement d'API de Google (un seul `projectId` étant possible dans une exécution) il faut dans ce cas utiliser un double export `firestore -> sqlite` puis `sqlite -> firestore`.

> Remarque: l'export de Storage `gc` (Google Cloud) vers `gc` étant techniquement limité par les contraintes d'environnement d'API de Google il faut dans ce cas utiliser un double export `gc -> fs (File-System)` puis `fs -> gc`.
